...
import { Folder, Image as ImageIcon, FileText, UploadCloud, Search, Trash2, Copy, CheckCircle2, Download, ExternalLink, X, Cloud, HardDrive } from "lucide-react";

// Interface for simulated media items
interface MediaItem {
  id: string;
  name: string;
  type: string;
...
export default function ClientMediaLibrary({ onSelectImage }: { onSelectImage?: (url: string) => void } = {}) {
  ...
  // Load from MySQL Database via /api/admin/media (with localStorage fallback)
  useEffect(() => {
    fetch("/api/admin/media", { credentials: "include" })
      .then(r => r.ok ? r.json() : [])
      .then(dbList => {
        if (Array.isArray(dbList) && dbList.length > 0) {
          const mapped = dbList.map((item: any) => ({
            id: String(item.id),
            name: item.name,
            type: item.type,
            size: item.size,
          ...
          if (saved) {
            try { setMediaList(JSON.parse(saved)); } catch (e) {}
          }
        }
      })
      .catch(() => {
  ...
  // Fetch media settings from System Admin API (and check if control is delegated to tenant)
  useEffect(() => {
    fetch("/api/media-settings")
      ...
      .then((data: MediaSettings & { allowTenantControl?: boolean }) => {
        if (data.allowTenantControl) {
          ...
          if (tenantSaved) {
            try {
              setMediaSettings(JSON.parse(tenantSaved));
              return;
            } catch (e) {}
          }
        }
      ...
      .catch(() => {
        ...
        if (saved) { try { setMediaSettings(JSON.parse(saved)); } catch (e) {} }
      });
  }, []);

  // Save to DB & localStorage
  const saveMedia = (newList: MediaItem[], addedItem?: MediaItem) => {
    ...
    if (addedItem) {
      ...
      }).catch(e => console.warn("Failed to save media to DB", e));
    }
    try {
      localStorage.setItem("tenant_media_library", JSON.stringify(newList));
    } catch (e) {
      console.warn("localStorage quota exceeded, storing lightweight media items", e);
      const lightList = newList.map((item) => ({
        ...item,
        url: item.url && item.url.length > 300000 ? "" : item.url,
      }));
      try {
        localStorage.setItem("tenant_media_library", JSON.stringify(lightList));
      } catch (err) {}
    }
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Enforce System Admin max file size limit
    const maxBytes = mediaSettings.maxFileSizeMB * 1024 * 1024;
    if (file.size > maxBytes) {
      showNotify(`Upload blocked: File exceeds the ${mediaSettings.maxFileSizeMB} MB limit set by System Admin.`);
    ...
    if (isPdf && !mediaSettings.allowPdf) { showNotify("PDF uploads are disabled by System Admin."); return; }
    if (isImg && !mediaSettings.allowImages) { showNotify("Image uploads are disabled by System Admin."); return; }
    if (isDoc && !mediaSettings.allowDocs) { showNotify("Document uploads are disabled by System Admin."); return; }

    const formattedSize = formatSize(file.size);
    setIsUploading(true);
    setUploadingFile({ name: file.name, size: formattedSize, progress: 5 });

    // Smooth animated progress increment
      ...
      reader.onloadend = () => {
        ...
        setTimeout(() => {
          try {
            ...
            const activeDriver = mediaSettings.storageDriver || "local";
            ...
            if (activeDriver === "s3") {
              const bucket = mediaSettings.s3Bucket || "sansuite-test-media-bucket";
              const region = mediaSettings.s3Region || "us-east-1";
              locationLabel = `AWS S3 Bucket (${bucket})`;
              directUrl = mediaSettings.s3Endpoint && mediaSettings.s3Endpoint.length > 5
                ? `${mediaSettings.s3Endpoint}/${bucket}/media/${file.name.toLowerCase().replace(/[^a-z0-9._-]/g, "_")}`
                : `https://${bucket}.s3.${region}.amazonaws.com/media/${file.name.toLowerCase().replace(/[^a-z0-9._-]/g, "_")}`;
            } else if (activeDriver === "cloudinary") {
              const cloud = mediaSettings.cloudinaryCloudName || "sansuite-cloud";
              ...
              const host = mediaSettings.sftpHost || "files.sansuite.com";
              locationLabel = `Remote SFTP Server (${host})`;
              directUrl = `http://${host}${mediaSettings.sftpPath || "/uploads/"}${file.name.toLowerCase().replace(/[^a-z0-9._-]/g, "_")}`;
            }
            ...
            const newItem: MediaItem = {
              ...
              category: isPdf ? "PDF Documents" : file.type.startsWith("image/") ? "Images" : "Documents",
              storageDriver: activeDriver,
              storageLocation: locationLabel,
            };

            saveMedia([newItem, ...mediaList], newItem);
            showNotify(`Asset uploaded to ${locationLabel}.`);
          } catch (err) {
            console.error("Upload save error:", err);
            showNotify("Asset uploaded.");
          } finally {
  ...
  const handleDelete = (id: string) => {
    if (confirm("Are you sure you want to permanently delete this asset from your firm?")) {
      const newList = mediaList.filter(m => m.id !== id);
      saveMedia(newList);
      fetch(`/api/admin/media/${id}`, { method: "DELETE", credentials: "include" }).catch(() => {});
  ...
  const copyToClipboard = (text: string) => {
    ...
    showNotify("URL copied to clipboard!");
  };
  ...
  // Build dynamic CATEGORIES from System Admin settings
  const CATEGORIES = [
    "All",
    ...(mediaSettings.allowPdf ? ["PDF Documents"] : []),
    ...(mediaSettings.allowDocs ? ["Documents"] : []),
    ...(mediaSettings.allowImages ? ["Images"] : []),
  ];

  return (
    <div className="flex h-[calc(100vh-170px)] gap-6 relative px-6 w-full">
      ...
      {uploadingFile && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl p-6 shadow-2xl border border-purple-100 max-w-sm w-full space-y-4 text-center">
            ...
            <p className="text-[11px] text-slate-400">Please wait while your file is securely stored.</p>
          </div>
        </div>
      )}
      ...
      <div className="w-64 shrink-0 flex flex-col gap-6">
        <div>
          <button 
            ...
            className="w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold py-3 px-4 rounded-xl shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-70"
          >
            {isUploading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex-1">
          <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 px-3 mt-2">Firm Folders</h3>
            ...
            {CATEGORIES.map(cat => (
              <button
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                  activeCategory === cat ? "bg-purple-50 text-purple-700" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"
                }`}
              >
                <Folder size={16} className={activeCategory === cat ? "text-purple-600" : "text-gray-400"} fill={activeCategory === cat ? "currentColor" : "none"} fillOpacity={0.2} />
                {cat}
              </button>
            ))}
          </div>
          ...
          <div className="mt-8 px-3">
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-3">
              <h4 className="text-xs font-bold text-blue-800 mb-1">Private Storage</h4>
              <p className="text-[10px] text-blue-600 leading-tight">These assets are strictly private to your firm. Only authorized staff can view them.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
            {activeCategory} <span className="text-sm font-normal text-gray-400">({filteredMedia.length})</span>
          </h2>
          <div className="relative w-64">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-purple-400 focus:ring-1 focus:ring-purple-400 shadow-sm transition-all"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {filteredMedia.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-gray-700 font-semibold mb-1">No files found</h3>
              <p className="text-sm text-gray-500 max-w-sm">There are no files in this category. Upload an asset to get started.</p>
            </div>
          ) : (
                ...
                return (
                  <div 
                    ...
                    className={`group relative aspect-square rounded-xl border overflow-hidden cursor-pointer transition-all ${
                      selectedItem?.id === item.id ? "border-purple-500 ring-2 ring-purple-500/20 shadow-md" : "border-slate-200 hover:border-purple-300 hover:shadow-sm"
                    }`}
                  >
                    {isImg ? (
      ...
      {selectedItem && (() => {
        ...
        const isZip = selectedItem.name.endsWith(".zip") || selectedItem.name.endsWith(".rar") || selectedItem.name.endsWith(".tar");

        const activeDriver = selectedItem.storageDriver || mediaSettings.storageDriver || "local";

        const cleanDirectLink = selectedItem.url && selectedItem.url.startsWith("http")
          ? selectedItem.url
          : activeDriver === "s3"
            ? (mediaSettings.s3Endpoint && mediaSettings.s3Endpoint.length > 5
                ? `${mediaSettings.s3Endpoint}/${mediaSettings.s3Bucket || "sansuite-test-media-bucket"}/media/${selectedItem.name.toLowerCase().replace(/[^a-z0-9._-]/g, "_")}`
                : `https://${mediaSettings.s3Bucket || "sansuite-test-media-bucket"}.s3.${mediaSettings.s3Region || "us-east-1"}.amazonaws.com/media/${selectedItem.name.toLowerCase().replace(/[^a-z0-9._-]/g, "_")}`)
            : activeDriver === "cloudinary"
              ? `https://res.cloudinary.com/${mediaSettings.cloudinaryCloudName || "sansuite-cloud"}/image/upload/v1/media/${selectedItem.name.toLowerCase().replace(/[^a-z0-9._-]/g, "_")}`
              : activeDriver === "sftp"
                ? `http://${mediaSettings.sftpHost || "files.sansuite.com"}${mediaSettings.sftpPath || "/uploads/"}${selectedItem.name.toLowerCase().replace(/[^a-z0-9._-]/g, "_")}`
                : `${window.location.origin}/uploads/media/${selectedItem.name.toLowerCase().replace(/[^a-z0-9._-]/g, "_")}`;

        const locationLabel = selectedItem.storageLocation || (
          activeDriver === "s3" ? `AWS S3 Bucket (${mediaSettings.s3Bucket || 'sansuite-test-media-bucket'})` :
          activeDriver === "cloudinary" ? `Cloudinary CDN (${mediaSettings.cloudinaryCloudName || 'sansuite-cloud'})` :
          activeDriver === "sftp" ? `Remote SFTP Server (${mediaSettings.sftpHost || 'files.sansuite.com'})` :
          "Local Server Storage"
        );

        return (
          <div className="w-80 shrink-0 bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col overflow-hidden animate-in slide-in-from-right-8 duration-300">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="font-bold text-gray-800">Asset Details</h3>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">File Name</label>
                  <p className="text-xs font-semibold text-gray-800 break-all">{selectedItem.name}</p>
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Storage Engine</label>
                  <div className="flex items-center gap-1.5 text-xs font-semibold text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-lg px-2.5 py-1">
                    {activeDriver === 's3' ? <Cloud size={14} className="text-indigo-600" /> : <HardDrive size={14} className="text-slate-600" />}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Size</label>
                    <p className="text-xs font-medium text-gray-700">{selectedItem.size}</p>
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Uploaded</label>
                    <p className="text-xs font-medium text-gray-700">{selectedItem.date}</p>
                  </div>
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Category & Type</label>
                  <p className="text-xs font-mono text-purple-700 font-semibold">{selectedItem.category} ({selectedItem.type})</p>
                </div>
                
                {/* Clean Direct Link Field */}
                <div className="pt-3 border-t border-gray-100">
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Direct Asset Link</label>
                  <div className="flex gap-2">
                    ...
                    <button 
                      ...
                      className="p-1.5 bg-purple-50 hover:bg-purple-100 text-purple-700 rounded-lg transition-colors border border-purple-200 font-semibold text-xs flex items-center gap-1"
                      title="Copy Direct Link"
                    >
                      <Copy size={13} />
                    </button>
                  </div>
                  <p className="text-[10px] text-gray-400 mt-1">Clean shareable asset URL for invoice templates and portal links.</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50 flex gap-2">
              {onSelectImage && (
                <button
                  type="button"
                  onClick={() => onSelectImage(selectedItem.url || cleanDirectLink)}
                  className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 rounded-lg text-xs flex items-center justify-center gap-1.5 transition-colors shadow-sm"
                >
                  <CheckCircle2 size={14} /> Attach as Firm Logo
                </button>
              )}
              ...
              <a 
                ...
                className={`${onSelectImage ? 'px-3 bg-purple-50 border border-purple-200 text-purple-700 hover:bg-purple-100' : 'flex-1 bg-purple-600 text-white hover:bg-purple-700'} font-semibold py-2 rounded-lg text-xs flex items-center justify-center gap-1.5 transition-colors shadow-sm`}
              >
