/**
 * Companies House API Enumerations
 * Sourced directly from official Companies House repository:
 * https://github.com/companieshouse/api-enumerations
 *
 * Provides human-readable labels for all Companies House codes:
 * - Company Types
 * - Company Status & Details
 * - Full SIC Descriptions (Standard Industrial Classification)
 * - Account Types
 * - Officer Roles
 * - Jurisdictions & Subtypes
 */

export const CH_COMPANY_TYPE: Record<string, string> = {
...
export const CH_COMPANY_STATUS: Record<string, string> = {
    "active": "Active",
    "dissolved": "Dissolved",
...
export const CH_COMPANY_STATUS_DETAIL: Record<string, string> = {
    "active": "",
    "dissolved": "",
    "converted-closed": "",
...
export const CH_ACCOUNT_TYPE: Record<string, string> = {
    "null": "Null",
    "full": "Full",
    "small": "Small",
    "medium": "Medium",
...
export const CH_OFFICER_ROLE: Record<string, string> = {
...
export const CH_COMPANY_SUBTYPE: Record<string, string> = {
    ...
    "slp": "Scottish Limited Partnership (SLP)",
    "pflp": "Private Fund Limited Partnership (PFLP)",
    "spflp": "Scottish Private Fund Limited Partnership (SPFLP)"
};

export const CH_JURISDICTION: Record<string, string> = {
...
export const CH_SIC_DESCRIPTIONS: Record<string, string> = {
...
/** Returns human-readable company type label. Falls back to formatted raw code. */
export function getCompanyTypeLabel(code: string | undefined | null): string {
    if (!code) return "—";
    const clean = code.trim().toLowerCase();
    return CH_COMPANY_TYPE[clean] || CH_COMPANY_TYPE[code] || capitalise(code.replace(/[-_]/g, " "));
}

/** Returns human-readable company status label. Falls back to formatted raw code. */
export function getCompanyStatusLabel(code: string | undefined | null): string {
    if (!code) return "—";
    const clean = code.trim().toLowerCase();
    return CH_COMPANY_STATUS[clean] || CH_COMPANY_STATUS[code] || capitalise(code.replace(/[-_]/g, " "));
}

/** Returns human-readable status detail if available. */
export function getCompanyStatusDetailLabel(code: string | undefined | null): string {
    if (!code) return "";
    const clean = code.trim().toLowerCase();
    return CH_COMPANY_STATUS_DETAIL[clean] || "";
}

/** Returns human-readable account type label. */
export function getAccountTypeLabel(code: string | undefined | null): string {
    if (!code) return "—";
    const clean = code.trim().toLowerCase();
    return CH_ACCOUNT_TYPE[clean] || CH_ACCOUNT_TYPE[code] || capitalise(code.replace(/[-_]/g, " "));
}

/** Returns human-readable officer role label. */
export function getOfficerRoleLabel(code: string | undefined | null): string {
    if (!code) return "—";
    const clean = code.trim().toLowerCase();
    return CH_OFFICER_ROLE[clean] || CH_OFFICER_ROLE[code] || capitalise(code.replace(/[-_]/g, " "));
}

/** Returns human-readable company subtype label. */
export function getCompanySubtypeLabel(code: string | undefined | null): string {
    if (!code) return "";
    const clean = code.trim().toLowerCase();
    return CH_COMPANY_SUBTYPE[clean] || "";
}

/** Returns human-readable jurisdiction label. */
export function getJurisdictionLabel(code: string | undefined | null): string {
    if (!code) return "—";
    const clean = code.trim().toLowerCase();
    return CH_JURISDICTION[clean] || capitalise(code.replace(/[-_]/g, " "));
}

/**
 * Returns SIC code formatted with human-readable description.
 * e.g. "62020 – Information technology consultancy activities"
 */
export function getSicLabel(code: string | undefined | null): string {
    if (!code) return "—";
    const trimmed = code.trim();
    const desc = CH_SIC_DESCRIPTIONS[trimmed];
    return desc ? `${trimmed} – ${desc}` : trimmed;
}

/**
 * Given comma or space separated SIC codes, returns an array of { code, description }.
 */
export function parseSicCodes(rawCodes: string | undefined | null): { code: string; description: string }[] {
    if (!rawCodes) return [];
    return rawCodes
        .split(/[,;\s]+/)
        .map(c => c.trim())
        ...
        .map(code => ({
            code,
            description: CH_SIC_DESCRIPTIONS[code] || "Standard UK Industry Classification"
        }));
}

/**
 * Returns Tailwind CSS badge classes for company status.
 */
export function getStatusBadgeClass(status: string | undefined | null): string {
    ...
    if (s.includes("dissolved") || s.includes("closed") || s.includes("removed")) {
        return "bg-rose-50 text-rose-700 border-rose-200";
    }
    if (s.includes("liquid") || s.includes("receiver") || s.includes("insolven")) {
        return "bg-amber-50 text-amber-700 border-amber-200";
    }
    if (s.includes("admin") || s.includes("voluntary")) {
